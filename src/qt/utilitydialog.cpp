...
InfoMessageDialog::InfoMessageDialog(QWidget *parent, HelpMode helpMode) :
    QDialog(parent),
    ui(new Ui::InfoMessageDialog)
{
    ui->setupUi(this);
...
void InfoMessageDialog::printToConsole()
{
    // On other operating systems, the expected action is to print the message to the console.
    fprintf(stdout, "%s\n", qPrintable(text));
}

void InfoMessageDialog::showOrPrint()
{
#if defined(WIN32)
    // On Windows, show a message box, as there is no stderr/stdout in windowed applications
    exec();
...
void InfoMessageDialog::on_okButton_accepted()
{
    close();
}
